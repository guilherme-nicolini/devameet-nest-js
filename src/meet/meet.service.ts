...
export class MeetService {
    ...
    async getMeetObject(meetId: string, userId: string) {
        this.logger.debug(`getmeetobject - ${userId} - ${meetId}`);
        const user = await this.userService.getUserById(userId);
    ...
    async update(meetId: string, userId: string, dto: UpdateMeetDto) {
        ...
        if (!meet) {
            throw new BadRequestException(MeetMessagesHelpers.UPDATE_MEET_NOT_FOUND);
        }
